...
import { getCountryCode, getFlag } from '@/lib/countryUtils';
...
export default function HomeClient({ stats, recentIncidents, recentVideos }: HomeClientProps) {
  ...
  const yearChartData = recentYears.map((y: any) => ({ year: y.year, count: y.count }));
  ...
  const getYouTubeVideoId = (incident: any) => {
    const allLinks = [
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Primary Causes */}
          <div 
            onClick={() => router.push('/dashboard/causes-analysis#primary-causes')}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={primaryCausesChartData} margin={{ left: 0, right: 0, top: 20, bottom: 60 }}>
                  ...
                  <Bar dataKey="percent" radius={[4, 4, 0, 0]} isAnimationActive={false}>
                    ...
                    <LabelList dataKey="percent" position="top" formatter={(v: any) => `${Number(v).toFixed(0)}%`} fill="#f1f5f9" style={{ fontSize: isMobile ? '9px' : '11px' }} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          {/* Flight Phase */}
          <div 
            onClick={() => router.push('/dashboard/flight-conditions#flight-phase')}
            className="bg-slate-900 rounded-xl p-4 md:p-6 border border-slate-800 cursor-pointer hover:border-amber-500/50 transition-all"
          >
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={flightPhaseChartData} margin={{ left: 0, right: 0, top: 20, bottom: 5 }}>
                  ...
                  <Bar dataKey="percent" fill="#fbbf24" radius={[4, 4, 0, 0]} isAnimationActive={false}>
                    <LabelList dataKey="percent" position="top" formatter={(v: any) => `${v.toFixed(0)}%`} fill="#f1f5f9" style={{ fontSize: isMobile ? '10px' : '12px' }} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          {/* Reserve Usage */}
          <div 
            onClick={() => router.push('/dashboard/safety-equipment#reserve-usage')}
            className="bg-slate-900 rounded-xl p-4 md:p-6 border border-slate-800 cursor-pointer hover:border-amber-500/50 transition-all"
          >
          ...
          {/* Recent Trends */}
          <div 
            onClick={() => router.push('/dashboard/trends-distribution#by-year')}
            className="bg-slate-900 rounded-xl p-4 md:p-6 border border-slate-800 cursor-pointer hover:border-amber-500/50 transition-all"
          >
